  ...
  <view class="page" v-if="order">
    ...
    <scroll-view class="content" scroll-y>
      ...
      <view class="section address-section" v-if="order.addressSnapshot">
        ...
        <text class="address-text">{{ order.addressSnapshot }}</text>
      </view>
      ...
      <view class="section price-section">
        ...
        <view class="price-row" v-if="order.couponAmount > 0"><text>优惠券抵扣</text><text style="color:#c0836a">-¥{{ order.couponAmount.toLocaleString() }}</text></view>
        <view class="price-row" v-if="order.pointsAmount > 0"><text>积分抵扣</text><text style="color:#c0836a">-¥{{ order.pointsAmount.toLocaleString() }}</text></view>
...
.top-bar { display: flex; align-items: center; height: 112rpx; padding: 0 32rpx; background: rgba(255,255,255,0.8); flex-shrink: 0; }
...
.content { flex: 1; padding: 0 32rpx 32rpx; }
.status-banner { border-radius: 20rpx; padding: 32rpx; margin-bottom: 16rpx; margin-top: 16rpx; }
...
.address-text { font-size: 26rpx; color: #605E5A; line-height: 1.6; }
...
.bottom-bar { position: sticky; bottom: 0; display: flex; justify-content: flex-end; gap: 16rpx; padding: 16rpx 32rpx; background: rgba(255,255,255,0.95); }
